...
holi <- vroom("US Federal Pay and Leave Holidays 2004 to 2100.csv")
```
...
```{r}
holi <- vroom("US Federal Pay and Leave Holidays 2004 to 2100.csv")
holi <- holi %>% select(Title, Date)
holi$new_date <- as.Date(holi$Date, format = "%m/%d/%Y")
```


## FULL JOIN 
```{r}
new_df <- full_join(holi, weather)
