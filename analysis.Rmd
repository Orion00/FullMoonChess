...
crimes_acf[max_crime_row,]

crimes_acf[max_crime_row, "Crime"] <- mean(crimes_acf$Crimes)
crimes$Year = year(as.Date(crimes$new_date))
crimes$Month = month(as.Date(crimes$new_date))
crimes$Day = day(as.Date(crimes$new_date))
...
# Homework Assignment - Replace that day with mean of crimes from other years
cc <- crimes %>%
  filter(Day == "31" & Month == "5" & Year != "2020") %>% group_by(new_date) %>% summarize(d = n())
cc
...
X <- model.matrix(Crimes~-1+isFullMoon+Holiday+feelslikemax+feelslikemin+dew+precip+snow+snowdepth+windgust+windspeed+cloudcover+visibility+weekend,data=crimes_acf)
...
test.set <- crimes_acf[4019:5168,] # Jan 1, 2021 - Feb 24, 2024
train.set <- crimes_acf[-(4019:5168),]
# ts_train <- ts(data=train.set$AnnAnom, start=c(1950,1), frequency=12)
ts_train <- zoo(train.set$Crimes, train.set$new_date)
X_train <- model.matrix(Crimes~-1+isFullMoon+Holiday+feelslikemax+feelslikemin+dew+precip+snow+snowdepth+windgust+windspeed+cloudcover+visibility+weekend,data=train.set)
X_test <- model.matrix(Crimes~-1+isFullMoon+Holiday+feelslikemax+feelslikemin+dew+precip+snow+snowdepth+windgust+windspeed+cloudcover+visibility+weekend,data=test.set)

# Fit
train.lm <- Arima(ts_train, order=c(3,0,2), seasonal=c(0,0,0), xreg=X_train)
