...
unique_ward <- names(table(crimes$Ward))[table(crimes$Ward) < 2139]
...
X <- model.matrix(Crimes~-1+windgust+Ward, data=crimes_acf)

crimes_acf$windgust <- as.numeric(crimes_acf$windgust) # No T/F?
...
X <- model.matrix(Crimes~-1+Ward,data=crimes_acf)
dim(X)
# Lower Half
# precip+preciptype+snow+snowdepth+windgust+windspeed+cloudcover+visibility+moonphase+weekend
# feelslikemax+feelslikemin+dew+precip+preciptype+snow+snowdepth+windgust+windspeed+cloudcover+visibility+moonphase+weekend

Arima(zoo_data, order=c(2,0,1), xreg=X)
