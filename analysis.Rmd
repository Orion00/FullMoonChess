...
```{r}
Crimes~-1+isFullMoon+Ward+Holiday+feelslikemax+feelslikemin+dew+precip+snow+snowdepth+windgust+windspeed+cloudcover+visibility+weekend
crimes_aa <- 
  crimes_acf <- crimes %>%
  group_by(day(new_date), month(new_date)) %>%
  summarise(
    Crimes = mean(x)
  )
...
X_predict <- (X_test)
my.preds <- forecast(c.lm, xreg=X_test, level=c(.95), h=nrow(X_test))
my.preds <- forecast(c.lm, level=c(.95), h=365)

plot(my.preds)

predict(c.lm,n.ahead=12)
```
